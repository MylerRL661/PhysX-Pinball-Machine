...
namespace VisualDebugger
{
	...
	void Init(const char *window_name, int width, int height)
	{
		...
		//changed camera starting position
		camera = new Camera(PxVec3(0.0f, 70.0f, 70.0f), PxVec3(0.f,-0.90f,-1.f), 10.f);
	...
	void HUDInit()
	{
		...
		hud.AddLine(EMPTY, "");
		//add a help screen
		hud.AddLine(HELP, " Pinball Simulation");
		hud.AddLine(HELP, " Pinball Simulation");
		hud.AddLine(HELP, " Pinball Simulation");
		//hud.AddLine(HELP, "    F9 - select next actor");
		
		hud.AddLine(HELP, "    8 - pause");
		hud.AddLine(HELP, "    9 - render mode");
		//controls
		hud.AddLine(HELP, "    1 - left flipper");
		hud.AddLine(HELP, "    2 - right flipper");
		hud.AddLine(HELP, "    3 - plunger");
		//add a pause screen
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "");
		hud.AddLine(PAUSE, "   Simulation paused. Press F10 to continue.");
		hud.AddLine(PAUSE, "   IF OUT OF LIVES - GAME OVER");
	...
	void RenderScene()
	{
		...
		//string for scores and lives, changes in the hud
		string displayScore = "Score: ";
		displayScore += std::to_string(scene->GetScore());
		hud.EditLine(HELP, displayScore, 0);

		string displayLives = "Lives: ";
		displayLives += std::to_string(scene->Lives);
		hud.EditLine(HELP, displayLives, 1);
	...
	void UserKeyPress(int key)
	{
		...
		{
		//implement your own
			//keys for flippers when pressed
		case '1':
			scene->FlipperLeftControl();
			break;
		case '2':
			scene->FlipperRightControl();
			break;
		case '3':
			scene->Plunge();
			break;
	...
	void UserKeyRelease(int key)
	{
		...
		{
		//implement your own
			//keys for flippers when released
		case '1':
			scene->FlipperLeftRelease();
			break;
		case '2':
			scene->FlipperRightRelease();
			break;
		case '3':
			scene->PlungerRelease();
			break;
		case 'R':
